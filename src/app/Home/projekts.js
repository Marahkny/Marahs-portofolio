      ...
      <div ref={ref} className="max-w-8xl px-6 mx-auto pb-40 z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 ">
          ...
          <motion.div style={{ y: col1Translate }} className="flex flex-col gap-6">
            <ProjectCard imgSrc="/photos/project 2.png" title="Techno" desc="UI/UX design & brand" />
          
          </motion.div>

          {/* 🟢 Andra kolumnen - Oscillerar vid scroll */}
          <motion.div style={{ y: col2Translate }} className="flex flex-col gap-6">
            <ProjectCard imgSrc="/photos/project 1.png" title="Framsteg förening" desc="Brand & website" />
...
function ProjectCard({ imgSrc, title, desc }) {
  return (
    <div className="mb-6 ">
      <img src={imgSrc} alt={title} className="w-full  object-contain rounded-[20px]" />
      <div className="flex justify-between items-center w-full mt-4 group">
        <div>
          <h3 className=" text-[24px] mb-2 group-hover:text-[#3B429F] transition-colors duration-300">{title}</h3>
          <p className="text-gray-300">{desc}</p>
        </div>
        <div className="flex items-center gap-2 group  cursor-pointer">
          <p className="text-[16px] text-gray-300">View</p>
          <LiaArrowRightSolid className="text-[#3B429F] text-[24px] transition-transform duration-300 group-hover:translate-x-2" />
        </div>
      </div>
    </div>
  );
